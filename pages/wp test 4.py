...
# 수정 1: 빨간 점 표시 제거 및 탭 명칭 깔끔하게 정리
            ...
            if selected_wells:
                ...
                if len(selected_wells) == 1:
                    ...
                    with st.expander(f"➕ Well [{pos}] 신규 처리 및 사진 작성", expanded=True):
                        ...
                        # 수정 2: '세포 유래/출처' 항목을 일반 정보 입력 컬럼과 일치시켜 배치 (중복 및 위치 겹침 해결)
...
def render_tab_management(selected_plate, treatments):
    # 수정 3: Wellplate 시각화 탭과 중복되는 상단 입력 영역 제거 및 전체 물질처리 이력 관리 전용 화면 구성
...
if not projects:
    ...
    if selected_plate:
        ...
        # 수정 1: 빨간점 표시 대신 깔끔한 st.segmented_control 적용
