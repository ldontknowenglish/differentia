...
def get_master_item_options():
    ...
    if hasattr(db, 'get_all_recipes'):
        ...
        for r in recipes:
            ...
            for d in details:
                if d.get("material_name"): mat_set.add(d.get("material_name"))
                if d.get("manufacturer"): mfg_set.add(d.get("manufacturer"))
                if d.get("cat_no"): cat_set.add(d.get("cat_no"))
...
with tab1:
    ...
    st.caption("💡 **한 줄 레이아웃**: 물질명, 제조사, Cat No.는 직접 입력하거나 이전에 사용한 목록에서 바로 선택할 수 있습니다.")
    ...
    # ---------------------------------------------------------
    # 🔥 기능 2: 이전 사용 물질/제조사/Cat No. 선택 및 직접입력 병행 (한 열 배치)
    # ---------------------------------------------------------
    for idx, item in enumerate(st.session_state.recipe_items):
        ...
        # 1. 물질명
        with c1:
            mat_val = st.text_input(f"mat_{idx}", value=item.get("material_name", ""), key=f"mat_txt_{idx}", label_visibility="collapsed", placeholder="예: RPMI1640")
            if master_mats:
                m_sel = st.selectbox(f"mat_sel_{idx}", ["기존 목록에서 선택..."] + master_mats, key=f"mat_pick_{idx}", label_visibility="collapsed")
                if m_sel != "기존 목록에서 선택...":
                    mat_val = m_sel

        # 2. 제조사
        with c2:
            mfg_val = st.text_input(f"mfg_{idx}", value=item.get("manufacturer", ""), key=f"mfg_txt_{idx}", label_visibility="collapsed", placeholder="예: Gibco")
            if master_mfgs:
                mfg_sel = st.selectbox(f"mfg_sel_{idx}", ["기존 목록에서 선택..."] + master_mfgs, key=f"mfg_pick_{idx}", label_visibility="collapsed")
                if mfg_sel != "기존 목록에서 선택...":
                    mfg_val = mfg_sel

        # 3. Cat No.
        with c3:
            cat_val = st.text_input(f"cat_{idx}", value=item.get("cat_no", ""), key=f"cat_txt_{idx}", label_visibility="collapsed", placeholder="예: 11875093")
            if master_cats:
                cat_sel = st.selectbox(f"cat_sel_{idx}", ["기존 목록에서 선택..."] + master_cats, key=f"cat_pick_{idx}", label_visibility="collapsed")
                if cat_sel != "기존 목록에서 선택...":
                    cat_val = cat_sel

        # 4. 용량/농도
        with c4:
            amt_val = st.text_input(f"amt_{idx}", value=item.get("amount", ""), key=f"amt_txt_{idx}", label_visibility="collapsed", placeholder="예: 500 mL / 10 mM")
