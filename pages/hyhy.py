...
with st.sidebar:
    ...
    # 기본 제공 프로젝트 목록 세션 초기화
    if "projects" not in st.session_state:
        st.session_state.projects = ["기본 프로젝트", "Project Alpha", "Project Beta"]
    ...
    new_project_name = st.text_input("프로젝트명 입력", placeholder="예: Project Gamma")
    if st.button("프로젝트 추가", use_container_width=True):
        if new_project_name.strip():
            clean_proj = new_project_name.strip()
            if clean_proj not in st.session_state.projects:
                st.session_state.projects.append(clean_proj)
                st.success(f"'{clean_proj}' 등록 완료!")
                st.rerun()
            else:
                st.warning("이미 존재하는 프로젝트명입니다.")
        else:
            st.error("프로젝트명을 입력해주세요.")

# ==========================================
# 📁 엑셀 저장 및 로드 헬퍼 함수 (프로젝트 경로 포함)
# ==========================================
...
def get_excel_filepath(project_name, batch_id, assay_name):
    # 프로젝트별 디렉토리 생성
    safe_project = str(project_name).replace("/", "_").replace("\\", "_").replace(" ", "_")
    project_dir = os.path.join(EXCEL_DIR, safe_project)
    if not os.path.exists(project_dir):
        os.makedirs(project_dir)

    safe_batch = str(batch_id).replace("/", "_").replace("\\", "_")
    safe_assay = str(assay_name).replace(" ", "_")
    return os.path.join(project_dir, f"{safe_batch}_{safe_assay}.xlsx")


def save_to_excel(project_name, batch_id, assay_name, df):
    filepath = get_excel_filepath(project_name, batch_id, assay_name)
    df.to_excel(filepath, index=False, engine="openpyxl")


def load_from_excel(project_name, batch_id, assay_name):
    filepath = get_excel_filepath(project_name, batch_id, assay_name)
...
st.caption(f"📂 현재 선택된 프로젝트: **{selected_project}**")
...
with col_right:
    ...
    if df_parsed is not None:
        ...
        if st.button(
            ...
            save_to_excel(selected_project, current_batch, "Cell_Count", final_df)
            st.toast(f"[{selected_project}] 프로젝트에 엑셀 파일이 성공적으로 저장되었습니다!", icon="📂")
            st.rerun()
...
# ==========================================
st.markdown(f"#### 📜 [{selected_project} / {current_batch}] 저장된 Cell Count 엑셀 이력")

saved_df = load_from_excel(selected_project, current_batch, "Cell_Count")
...
if not saved_df.empty:
    ...
    with col_b1:
        if st.button("🔄 엑셀 파일 수정사항 업데이트"):
            ...
            save_to_excel(selected_project, current_batch, "Cell_Count", re_calculated_df)
    ...
    with col_b2:
        ...
        st.download_button(
            ...
            data=excel_data,
            file_name=f"{selected_project}_{current_batch}_Cell_Count.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
else:
    st.info("현재 선택된 프로젝트 및 배치 ID에 저장된 엑셀 파일이 없습니다.")
