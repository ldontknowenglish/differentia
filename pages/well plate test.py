...
if not projects:
    ...
    if st.button("처리 내역 및 사진 저장", use_container_width=True, type="primary", key="btn_t_main_save"):
        if t_well.strip() and t_comps:
            ...
            img_b64 = file_to_base64(t_file) if t_file else None
    ...
    # ---------------------------------------------------------
    # 2. 전체 물질 처리 이력 관리 (조회 / 수정 / 선택 삭제)
    # ---------------------------------------------------------
    st.subheader("📋 전체 물질 처리 이력 관리 (수정/삭제/사진관리)")

    if treatments:
        # 삭제 확인 팝업 모달 정의
        @st.dialog("⚠️ 선택 항목 일괄 삭제")
        def confirm_delete_dialog(ids_to_delete):
            st.warning(f"선택하신 **{len(ids_to_delete)}개**의 항목을 정말 삭제하시겠습니까?\n삭제된 데이터는 복구할 수 없습니다.")
            ...
            with c_del1:
                if st.button("예, 삭제합니다", type="primary", use_container_width=True):
                    ...
                    st.toast(f"{len(ids_to_delete)}개 항목이 삭제되었습니다.", icon="🗑️")
                    st.rerun()
            with c_del2:
        ...
        with top_c3:
            if st.button("🗑️ 선택 항목 일괄 삭제", type="primary", use_container_width=True, disabled=(len(selected_ids) == 0)):
                confirm_delete_dialog(selected_ids)
