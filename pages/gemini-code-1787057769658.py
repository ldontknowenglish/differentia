...
# 입력 관련 세션 스테이트 키 초기화 함수 (첫 번째 항목 불러오기 오류 방지)
def clear_recipe_input_keys():
    keys_to_delete = [k for k in st.session_state.keys() if k.startswith(("mat_", "mfg_", "cat_", "amt_"))]
...
with tab1:
        ...
        if not all_existing_recipes:
            ...
            with col_load2:
                ...
                if st.button("📥 레시피 가져오기", use_container_width=True, type="secondary"):
                    ...
                    if fetched_details:
                        # 💡 기존 동적 입력 키를 초기화하여 첫 번째 항목부터 완벽히 동기화
                        clear_recipe_input_keys()
                        ...
                        st.success(f"'{selected_recipe_label}'의 구성 물질 항목을 불러왔습니다!")
                        st.rerun()
    ...
    st.caption("💡 **추천 검색 기능**: 드롭다운 클릭 후 단어(예: `RP`)를 타이핑하면 관련 항목이 추천되며, 목록에 없을 경우 `➕ 직접 입력`을 선택해 작성할 수 있습니다.")
    ...
    # ---------------------------------------------------------
    # 🔥 기능 2: 추천 검색어(자동 필터링 선택) & 직접 입력 지원
    # ---------------------------------------------------------
    ...
    for idx, item in enumerate(st.session_state.recipe_items):
        ...
        with c1:
            ...
            # 검색어 팝업 지원 선택 상자 (키보드 타이핑 시 자동 추천)
            mat_idx = m_options.index(cur_mat) if cur_mat in m_options else 0
            selected_mat = st.selectbox(f"mat_sel_{idx}", options=m_options, index=mat_idx, key=f"mat_sel_key_{idx}", label_visibility="collapsed")
            
            if selected_mat == DIRECT_INPUT_OPT:
                mat_val = st.text_input(f"mat_txt_{idx}", value="" if cur_mat in master_mats else cur_mat, key=f"mat_txt_key_{idx}", label_visibility="collapsed", placeholder="물질명 직접 입력")
            else:
        ...
        with c2:
            ...
            selected_mfg = st.selectbox(f"mfg_sel_{idx}", options=mfg_options, index=mfg_idx, key=f"mfg_sel_key_{idx}", label_visibility="collapsed")
            
            if selected_mfg == DIRECT_INPUT_OPT:
                mfg_val = st.text_input(f"mfg_txt_{idx}", value="" if cur_mfg in master_mfgs else cur_mfg, key=f"mfg_txt_key_{idx}", label_visibility="collapsed", placeholder="제조사 직접 입력")
            else:
        ...
        with c3:
            ...
            selected_cat = st.selectbox(f"cat_sel_{idx}", options=cat_options_list, index=cat_idx, key=f"cat_sel_key_{idx}", label_visibility="collapsed")
            
            if selected_cat == DIRECT_INPUT_OPT:
                cat_val = st.text_input(f"cat_txt_{idx}", value="" if cur_cat in master_cats else cur_cat, key=f"cat_txt_key_{idx}", label_visibility="collapsed", placeholder="Cat No. 직접 입력")
            else:
                cat_val = selected_cat

        # 4. 용량/농도
        with c4:
            amt_val = st.text_input(f"amt_txt_{idx}", value=item.get("amount", ""), key=f"amt_txt_key_{idx}", label_visibility="collapsed", placeholder="예: 500 mL / 10 mM")
