...
# 활성 탭 관리를 위한 Session State 초기화
if "active_tab" not in st.session_state:
    st.session_state.active_tab = "📋 전체 플레이트 개요 (Overview)"
...
def generate_project_cell_lineage_dot(project_id):
    ...
    # [수정 3] 이미지 및 노드 사이즈 축소를 위한 파라미터 조절 (nodesep, ranksep, font, height 설정)
    dot_lines = [
...
def render_tab_overview():
    ...
    # [수정 1] 편집하러 가기 클릭 시 "🔴 Well Plate 시각화 & 편집" 탭으로 활성화
    def navigate_to_plate(proj_label, pl_key, proj_name, pl_name):
        ...
        st.session_state.active_tab = "🔴 Well Plate 시각화 & 편집"
...
def render_tab_visualization(selected_plate, treatments):
    ...
    with left_col:
        ...
        # [수정 2] on_select 동작 최적화 및 선택값 세션 동기화
    ...
    with right_col:
        ...
        with edit_main_tab1:
            ...
            # [수정 2] Plotly 웰 클릭 시 화면 전체가 초기화되지 않고 multiselect에만 안전하게 반영되도록 보정
...
# ======================================================================
# 요구사항 3 반영: 계통도 탭 Render 함수 (크기 제한 CSS 적용)
# ======================================================================
def render_tab_tree(selected_project):
    ...
    if dot_code:
        # [수정 3] 계통도 이미지 크기가 너무 커지지 않도록 스타일을 감싸서 렌더링
...
# ======================================================================
# 요구사항 4 반영: 날짜별 관리 탭 Render 함수 (전체 선택/해제 및 일괄 삭제)
# ======================================================================
...
if not projects:
    ...
    if selected_plate:
        ...
        # [수정 1] 외부에서 탭 전환 명령을 수용할 수 있는 커스텀 네비게이션
        ...
        if selected_tab == "📋 전체 플레이트 개요 (Overview)":
            render_tab_overview()
        elif selected_tab == "🔴 Well Plate 시각화 & 편집":
            render_tab_visualization(selected_plate, treatments)
        elif selected_tab == "🌳 프로젝트 통합 세포 계통도":
            render_tab_tree(selected_proj)
        elif selected_tab == "📝 날짜별 물질/세포 처리 입력 및 전체 관리":
