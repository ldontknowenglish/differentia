...
if not projects:
    ...
    if selected_plate:
        ...
        # ======================================================================
        with tab_log:
            st.caption(f"💡 현재 선택된 프로젝트(**{selected_proj['name']}**)의 관찰 일지 및 연구 기록을 관리합니다.")

            with st.container(border=True):
                st.markdown("##### 📝 새 데일리 로그 작성")
                with st.form("add_log_form", clear_on_submit=True):
                    log_date = st.date_input("로그 일자", datetime.date.today(), key="daily_log_date_input")
                    log_content = st.text_area("연구 관찰 및 기록 내용", placeholder="오늘의 관찰 결과, 세포 상태, 비고 사항 기록", key="daily_log_content_input")
