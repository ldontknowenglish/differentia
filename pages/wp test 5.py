...
# 수정 2: 라벨 옆에 물질 개수 입력을 나란히 배치하여 공간 절약
            ...
            if selected_wells:
                ...
                if len(selected_wells) == 1:
                    ...
                    with st.expander(f"➕ Well [{pos}] 신규 처리 및 사진 작성", expanded=True):
                        ...
                        # 수정 1: 상단 3개 컬럼에 세포 유래/출처 추가 및 분석진행 상태 위치 조정
                        ...
                        ex_analysis = st.selectbox("🔬 분석진행 상태", options=st.session_state.analysis_options, key=f"ex_analysis_{pos}")
                        
                        # 수정 2: basal media를 처리물질 및 농도 아래에 위치하고 공간 절약
                        if ex_analysis == "미진행":
                    ...
                    if items:
                        ...
                        for item in items:
                            ...
                            with st.expander(f"📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} | 🧪 {formatted_cond}"):
                                ...
                                # 수정 2: 개별 항목 수정 영역에서도 basal media를 처리 물질 아래에 배치
                                if mod_analysis == "미진행":
                    ...
                    b_analysis = st.selectbox("🔬 분석진행 상태", options=st.session_state.analysis_options, key="batch_analysis")
                    
                    # 수정 2: 다중 선택 일괄 처리에서도 basal media 위치 개선
                    if b_analysis == "미진행":
        ...
        for idx, item in enumerate(treatments):
            with list_cols[idx % 2]:
                ...
                with st.expander(f"{img_flag}📍 Well [{item['well_position']}] | 📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} ({analysis_lbl}) | 🧪 {formatted_cond}"):
                    ...
                    # 수정 2: 전체 이력 관리 세부 수정에서도 basal media 위치 개선
                    if e_analysis == "미진행":
                        e_comps = [c.strip() for c in str(item['compound_name']).split(',') if c.strip()]
                        e_concs = [c.strip() for c in str(item['concentration']).split(',')] if item['concentration'] else []
