...
if treatments:
    # ---------------------------------------------------------
    # [추가] 일괄 삭제 제어 영역 (전체 선택 & 일괄 삭제 버튼)
    # ---------------------------------------------------------
    top_c1, top_c2, top_c3 = st.columns([1.5, 2, 1.5])
    
    with top_c1:
        # 전체 선택/해제 토글 체크박스
        select_all = st.checkbox("전체 선택", key="t_select_all_toggle")
        if select_all:
            for item in treatments:
                st.session_state[f"chk_sel_t_{item['id']}"] = True
        else:
            # 전체 선택 해제 시 (단, 사용자가 개별 클릭한 상태는 유지하고 싶다면 이 블록을 제거/조정 가능)
            pass

    # 현재 체크된 항목 ID 추출
    selected_ids = [item['id'] for item in treatments if st.session_state.get(f"chk_sel_t_{item['id']}", False)]
    
    with top_c2:
        st.markdown(f"**선택된 항목:** `{len(selected_ids)}` / `{len(treatments)}` 개")
        
    with top_c3:
        # 일괄 삭제 버튼 (선택된 항목이 없으면 비활성화)
        if st.button("🗑️ 선택 항목 일괄 삭제", type="primary", use_container_width=True, disabled=(len(selected_ids) == 0)):
            for tid in selected_ids:
                db.delete_treatment(tid)
                # 세션 데이터 정리
                if f"chk_sel_t_{tid}" in st.session_state:
                    del st.session_state[f"chk_sel_t_{tid}"]
            st.toast(f"{len(selected_ids)}개 항목이 삭제되었습니다.", icon="🗑️")
            st.rerun()
    ...
    for idx, item in enumerate(treatments):
        with list_cols[idx % 2]:
            ...
            # [추가] 카드 상단/Expander 레이아웃 분리 (선택 체크박스 + Expander 배치)
            card_col1, card_col2 = st.columns([0.1, 0.9])
            
            with card_col1:
                # 각 항목별 선택 체크박스
                st.checkbox("", key=f"chk_sel_t_{item['id']}", label_visibility="collapsed")
                ...
                with st.expander(f"{img_flag}📍 Well [{item['well_position']}] | 📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} ({analysis_lbl}) | 🧪 {formatted_cond}"):
                    ...
                    with btn_c2:
                        if st.button("🗑️ 삭제", key=f"btn_t_del_{item['id']}", type="secondary", use_container_width=True):
                            db.delete_treatment(item['id'])
                            # 삭제 후 세션 삭제
                            if f"chk_sel_t_{item['id']}" in st.session_state:
                                del st.session_state[f"chk_sel_t_{item['id']}"]
                            st.toast("삭제되었습니다.", icon="🗑️")
                            st.rerun()
else:
