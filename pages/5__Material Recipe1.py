...
# 💡 레시피 불러오기 시 UI 위젯을 완전 강제 재렌더링하기 위한 버전 카운터
if "recipe_load_ver" not in st.session_state:
    st.session_state.recipe_load_ver = 0
...
with tab1:
        ...
        if not all_existing_recipes:
            ...
            with col_load2:
                ...
                if st.button("📥 레시피 가져오기", use_container_width=True, type="primary"):
                    ...
                    if fetched_details:
                        # 💡 버전 번호를 올려서 이전 위젯 Key들을 완전히 무효화함
                        st.session_state.recipe_load_ver += 1
                        ...
                        st.success(f"'{selected_recipe_label}' 데이터를 성공적으로 불러왔습니다!")
                        st.rerun()

    col1, col2, col3 = st.columns([2, 1.5, 1.5])
    with col1:
        recipe_name = st.text_input("레시피/배지 이름", placeholder="예: DE basal media", key=f"r_name_{st.session_state.recipe_load_ver}")
    
    with col2:
        cat_options = st.session_state.custom_categories if st.session_state.custom_categories else ["미분류"]
        category = st.selectbox("분류 (카테고리)", cat_options, key=f"r_cat_{st.session_state.recipe_load_ver}")

    with col3:
        prepared_date = st.date_input("조제/만든 날짜", value=datetime.date.today(), key=f"r_date_{st.session_state.recipe_load_ver}")

    description = st.text_input("설명 / 비고", placeholder="예: Definitive Endoderm 유도용 기초 배지 (10일 보관 가능)", key=f"r_desc_{st.session_state.recipe_load_ver}")
    ...
    ver = st.session_state.recipe_load_ver  # 현재 버전 코드
    ...
    for idx, item in enumerate(st.session_state.recipe_items):
        ...
        # 1. 물질명
        with c1:
            cur_mat = item.get("material_name", "").strip()
        ...
        # 2. 제조사
        with c2:
            cur_mfg = item.get("manufacturer", "").strip()
        ...
        with c4:
            amt_val = st.text_input(
                f"amt_txt_{ver}_{idx}",
                value=item.get("amount", ""),
                key=f"amt_txt_key_{ver}_{idx}",
                label_visibility="collapsed",
        ...
        # 5. 행 삭제 버튼
        with c5:
            if st.button("🗑️", key=f"del_btn_{ver}_{idx}"):
    ...
    with col_add:
        if st.button("➕ 물질 항목 추가", key=f"add_row_btn_{ver}"):
            st.session_state.recipe_items.append({"material_name": "", "manufacturer": "", "cat_no": "", "amount": ""})
            st.rerun()

    st.markdown("---")
    if st.button("💾 레시피 DB에 저장", type="primary", use_container_width=True, key=f"save_recipe_btn_{ver}"):
        if not recipe_name:
