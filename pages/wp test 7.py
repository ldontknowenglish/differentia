...
TAB_OVERVIEW = "📋 전체 플레이트 개요"
TAB_VISUALIZATION = "🧫 Well Plate 시각화 & 편집"
TAB_TREE = "🌳 프로젝트 통합 세포 계통도"
TAB_MANAGEMENT = "📋 전체 물질 처리 이력 관리"

if "active_tab" not in st.session_state or st.session_state.active_tab not in [TAB_OVERVIEW, TAB_VISUALIZATION, TAB_TREE, TAB_MANAGEMENT]:
...
def get_project_plate_wells_options(project_id, current_val=""):
    ...
    if plates:
        for pl in plates:
            ...
            well_cell_map = {
                t.get('well_position', '').upper(): t.get('cell_info', '').strip()
                for t in t_list if t.get('well_position') and t.get('cell_info')
            }
...
def render_compound_inputs(key_prefix, default_comps=None, default_concs=None):
    ...
    hdr_c1.caption("🧪 **처리 물질 및 농도**")
    num_pairs = hdr_c2.number_input(
        "물질 개수", min_value=1, max_value=10, 
        value=max(1, len(default_comps), 2), key=f"{key_prefix}_num_pairs", label_visibility="collapsed"
    )
...
def generate_project_cell_lineage_dot(project_id):
        ...
        if "[PassageFrom:" in note:
            try:
                ...
                for existing_cell, info in nodes_info.items():
                    ...
                    if (is_matched_cell or is_matched_loc) and existing_cell != c_info:
    ...
    dot_lines = [
        ...
        "    graph [nodesep=0.25, ranksep=0.4, margin=0, pad=0.1, splines=ortho];",
        "    node [shape=box, style=\"filled,rounded\", fontname=\"Malgun Gothic, sans-serif\", fontsize=8, height=0.25, margin=\"0.1,0.05\"];",
        "    edge [color=\"#64748b\", arrowhead=normal, arrowsize=0.5, penwidth=1.0, fontname=\"Malgun Gothic, sans-serif\", fontsize=7];"
    ]
    ...
    for cell_group, info in nodes_info.items():
        ...
        if cell_group in root_cells:
            label = f"👑 {clean_name}\\n({first_date})"
            dot_lines.append(
                f'    "{clean_name}" [label="{label}", fillcolor="#fef9c3", color="#ca8a04", penwidth=1.5, fontcolor="#854d0e"];'
            )
        else:
            label = f"🧬 {clean_name}\\n({plates_str})"
            dot_lines.append(
                f'    "{clean_name}" [label="{label}", fillcolor="#eff6ff", color="#2563eb", penwidth=1.0, fontcolor="#1e3a8a"];'
            )
...
def render_tab_overview():
    ...
    def navigate_to_plate(proj_label, pl_key, proj_name, pl_name):
        ...
        st.session_state.active_tab = TAB_VISUALIZATION
...
def render_tab_visualization(selected_plate, treatments):
    ...
    with left_col:
        ...
        # 마커/폰트 사이즈 조건 정리
        if total_wells <= 6: marker_size, font_size = 65, 12
        elif total_wells <= 12: marker_size, font_size = 55, 11
        elif total_wells <= 24: marker_size, font_size = 48, 10
        elif total_wells <= 48: marker_size, font_size = 40, 9
        else: marker_size, font_size = 32, 8
        ...
        plotly_event = st.plotly_chart(
            ...
            key=f"plate_chart_{selected_plate['id']}"
        )
    ...
    with right_col:
        ...
        with edit_main_tab1:
            ...
            if current_sig and current_sig != st.session_state.get("last_dragged_signature"):
                ...
                if len(selected_wells) == 1:
                    ...
                    with st.expander(f"➕ Well [{pos}] 신규 처리 및 사진 작성", expanded=True):
                        ...
                        if is_transfer:
                            ...
                            ex_psg_src = p_col1.text_input("출처 세포/Well 직접 입력", value="DE_1 (A1)", key=f"src_p_input_{pos}") if selected_src_opt == "직접 입력 / 해당 없음" else selected_src_opt
                        ...
                        ex_analysis = c3.selectbox("🔬 분석 진행상태", options=st.session_state.analysis_options, key=f"ex_analysis_{pos}")
                        ...
                        if st.button(f"💾 Well [{pos}] 처리 저장", key=f"btn_ex_save_{pos}", use_container_width=True, type="primary"):
                            if ex_analysis != "미진행" or ex_comp_str.strip():
                                comb_note = build_combined_note(ex_basal, "", ex_psg_src, ex_psg_reagent, ex_psg_seeding, ex_note, file_to_base64(ex_file))
                    ...
                    if items:
                        ...
                        for item in items:
                            ...
                            with st.expander(f"📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} | 🧪 {formatted_cond}"):
                                ...
                                b_media_val, _, psg_src_v, psg_reagent_v, psg_seeding_v, pure_note_val, cur_img_b64 = parse_note_basal_image(item)

                                m_col1, m_col2, m_col3 = st.columns(3)
                                mod_d = m_col1.date_input("처리 일자", value=def_d, key=f"s_date_{item['id']}")
                                mod_cell = m_col2.text_input("세포 정보", value=item.get('cell_info', ''), key=f"s_cell_{item['id']}")
                                cur_a = item.get('analysis_status', '미진행')
                                analysis_opts = st.session_state.analysis_options
                                mod_analysis = m_col3.selectbox("🔬 분석 진행상태", options=analysis_opts, index=analysis_opts.index(cur_a) if cur_a in analysis_opts else 0, key=f"s_analysis_{item['id']}")

                                m_col4, m_col5, m_col6 = st.columns(3)
                                mod_pos = m_col4.text_input("웰 위치", value=item['well_position'], key=f"s_pos_{item['id']}")
                                ...
                                mod_psg_seeding = st.text_input("시딩 세포수", value=psg_seeding_v, key=f"s_psg_seeding_{item['id']}")
                                ...
                                if b_save.button("💾 저장", key=f"btn_s_save_{item['id']}", type="primary", use_container_width=True):
                                    ...
                                    comb_note = build_combined_note(mod_basal, "", mod_psg_src, mod_psg_reagent, mod_psg_seeding, mod_note, final_img)
                    ...
                    if batch_trans:
                        ...
                        b_psg_src = bp_col1.text_input("출처 세포/Well 직접 입력", value="DE_1", key="batch_src_input") if selected_b_src == "직접 입력 / 해당 없음" else selected_b_src
                    ...
                    b_analysis = bc3.selectbox("🔬 분석 진행상태", options=st.session_state.analysis_options, key="batch_analysis")
                    ...
                    if st.button(f"💾 선택한 {len(selected_wells)}개 Well에 일괄 저장", key="btn_batch_save", use_container_width=True, type="primary"):
                        if b_comp_str.strip():
                            comb_note = build_combined_note(b_basal, "", b_psg_src, b_psg_reagent, b_psg_seeding, b_note, file_to_base64(b_file))
...
def render_tab_tree(selected_project):
    ...
    if dot_code:
        st.markdown("<div style='max-height: 480px; overflow-x: auto; overflow-y: auto; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px; background: white;'>", unsafe_allow_html=True)
...
def render_tab_management(selected_plate, treatments):
    ...
    if treatments:
        if "select_all_treatments" not in st.session_state:
            st.session_state.select_all_treatments = False

        records = []
        for item in treatments:
            b_media, _, psg_src, psg_reagent, psg_seeding, pure_note, img_b64 = parse_note_basal_image(item)
            records.append({
                "ID": item['id'],
                "선택": st.session_state.select_all_treatments,
                "웰 위치": item['well_position'],
                "일자": item['treatment_date'],
                "세포 정보": item.get('cell_info', ''),
                "분석 진행상태": item.get('analysis_status', '미진행'),
                "Basal Media": b_media,
                "계대 출처": psg_src,
                "해취 물질": psg_reagent,
                "시딩 수": psg_seeding,
        ...
        chk_col1, _ = st.columns([2, 8])
        with chk_col1:
            def toggle_select_all():
                st.session_state.select_all_treatments = st.session_state.chk_select_all_val

            st.checkbox(
                "☑️ 전체 선택 / 해제", 
                value=st.session_state.select_all_treatments, 
                key="chk_select_all_val", 
        ...
        if selected_count > 0:
            ...
            with m_col1:
                if st.button(f"🗑️ 선택한 {selected_count}개 항목 일괄 삭제", type="primary", use_container_width=True, key="btn_delete_selected"):
                    for _, row in selected_rows.iterrows():
                        db.delete_treatment(row["ID"])
                    st.toast(f"{selected_count}개 항목이 성공적으로 삭제되었습니다.", icon="🗑️")
                    st.session_state.select_all_treatments = False
                    st.rerun()
            ...
            with m_col2:
                with st.popover(f"✏️ 선택한 {selected_count}개 항목 일괄 수정", use_container_width=True):
                    ...
                    mod_analysis = b_col3.selectbox("분석 진행상태", options=["(변경 안 함)"] + st.session_state.analysis_options, key="m_batch_analysis")
                    ...
                    mod_comp = st.text_input("변경할 물질명", placeholder="예: VEGF", key="m_batch_comp")

                    if st.button("💾 일괄 수정 사항 적용", type="primary", use_container_width=True):
                        for _, row in selected_rows.iterrows():
                            item_id = row["ID"]
                            orig_item = next(t for t in treatments if t['id'] == item_id)
                            b_media_val, _, psg_src_v, psg_reagent_v, psg_seeding_v, pure_note_val, cur_img_b64 = parse_note_basal_image(orig_item)
                            ...
                            comb_note = build_combined_note(b_media_val, "", final_psg_src, final_psg_reagent, final_psg_seeding, pure_note_val, cur_img_b64)
        ...
        for idx, item in enumerate(treatments):
            with list_cols[idx % 2]:
                ...
                with st.expander(f"{img_flag}📍 Well [{item['well_position']}] | 📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} ({analysis_lbl}) | 🧪 {formatted_cond}"):
                    ...
                    e_cell = e_c2.text_input("세포 정보", value=item.get('cell_info', ''), key=f"t_e_cell_{item['id']}")
                    e_cur_a = item.get('analysis_status', '미진행')
                    analysis_opts = st.session_state.analysis_options
                    e_analysis = e_c3.selectbox("🔬 분석 진행상태", options=analysis_opts, index=analysis_opts.index(e_cur_a) if e_cur_a in analysis_opts else 0, key=f"t_e_analysis_{item['id']}")

                    e_c4, e_c5, e_c6 = st.columns(3)
                    e_pos = e_c4.text_input("웰 위치", value=item['well_position'], key=f"t_e_pos_{item['id']}")
                    ...
                    e_psg_seeding = st.text_input("시딩 세포수", value=psg_seeding_v, key=f"t_e_psg_seeding_{item['id']}")
                    ...
                    if b_c1.button("💾 저장", key=f"btn_t_update_{item['id']}", use_container_width=True):
                        ...
                        comb_note = build_combined_note(e_basal, "", e_psg_src, e_psg_reagent, e_psg_seeding, e_note, final_img)
...
if not projects:
    ...
    if selected_plate:
        ...
        if selected_tab == TAB_OVERVIEW:
            render_tab_overview()
        elif selected_tab == TAB_VISUALIZATION:
            render_tab_visualization(selected_plate, treatments)
        elif selected_tab == TAB_TREE:
            render_tab_tree(selected_proj)
        elif selected_tab == TAB_MANAGEMENT:
