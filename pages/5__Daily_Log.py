...
if not projects:
    ...
    with st.form("add_log_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_button("로그 추가", use_container_width=True)
        if submitted:
    ...
    if logs:
        st.caption(f"💡 총 {len(logs)}건 · 한 화면에서 보기 편하도록 2개씩 나란히 표시됩니다.")
            ...
            with log_cols[idx % 2]:
                with st.container(border=True):
                    ...
                    if st.button("🗑️ 휴지통으로 이동", key=f"btn_del_log_{log['id']}", use_container_width=True):
                        ...
                        st.toast("로그가 휴지통으로 이동되었습니다. (📥 휴지통에서 복구 가능)", icon="🗑️")
                        st.rerun()
    else:
