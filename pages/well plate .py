...
from well_plate_utils import (
    ...
    extract_image_data,
    get_basal_media,
    parse_note_basal_image,
    build_combined_note,
    get_recipe_options,
    format_compound_summary,
    generate_dynamic_lineage_dot
)
...
try:
    ...
    st.error(f"⚠️ 데이터베이스를 초기화하는 중 오류가 발생했습니다: {e}")
    projects = []
                            ...
                            if pos in well_all_map:
                                    ...
                                    with st.expander(f"📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} | 🧪 {formatted_cond}", expanded=True):
                                        ...
                                        with r2_c2:
                                            if cur_s_analysis == "미진행":
                                                b_opts = get_recipe_options(db, b_media_val)
                                ...
                                with st.expander(f"➕ Well [{pos}]에 추가 처리 및 사진 작성", expanded=False):
                                    ...
                                    with r2_c2:
                                        if ex_analysis_val == "미진행":
                                            ex_basal = st.selectbox("Basal Media (레시피 선택)", options=get_recipe_options(db), key=f"ex_basal_{pos}")
                                        else:
                                            ex_basal = "-"
                                ...
                                with r2_c2:
                                    ...
                                    if e_analysis_val == "미진행":
                                        e_basal = st.selectbox("Basal Media (레시피 선택)", options=get_recipe_options(db), key=f"e_basal_{pos}")
                                    else:
                                        e_basal = "-"
                            ...
                            with tab_sub_batch:
                                ...
                                with r2_c2:
                                    ...
                                    if batch_analysis_val == "미진행":
                                        b_basal = st.selectbox("Basal Media (레시피 선택)", options=get_recipe_options(db), key="batch_basal")
                                    else:
                                        b_basal = "-"
        ...
        with tab_treat:
            ...
            with r2_c2:
                if t_analysis_val == "미진행":
                    t_basal = st.selectbox("Basal Media (레시피 선택)", options=get_recipe_options(db), key="t_basal_main")
                else:
                    t_basal = "-"
                        ...
                        with st.expander(f"{img_flag}📍 Well [{item['well_position']}] | 📅 {item['treatment_date']} | 🧬 {item.get('cell_info', '-')} ({analysis_lbl}) | 🧪 {formatted_cond}"):
                            ...
                            with r2_c2:
                                if e_cur_s_analysis == "미진행":
                                    e_b_opts = get_recipe_options(db, b_media_val)
