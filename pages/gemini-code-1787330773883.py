...
def generate_dynamic_lineage_dot(treatments):
    ...
    for well, group in df.groupby('well_position'):
        ...
        for _, row in group.iterrows():
            ...
            if c_info and (not cell_history or cell_history[-1][0] != c_info):
                cell_history.append((c_info, t_date))
        
        for c_info, _ in cell_history: nodes.add(c_info)

        for i in range(len(cell_history) - 1):
            src, dst, dst_date = cell_history[i][0], cell_history[i + 1][0], cell_history[i + 1][1]
            if src != dst: edges.add((src, dst, dst_date))
    ...
    for src, dst, transition_date in edges:
        ...
        dot_lines.append(f'    "{c_src}" -> "{c_dst}" [label=" {c_date} ", fontcolor="#475569"];')
